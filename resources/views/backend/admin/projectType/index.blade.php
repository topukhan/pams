    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="px-4 ">
            <ol class="flex justify-end text-gray-500">
                ...
                <li class="mr-3">/ </li>
                <li>
                    <a href="{{ route('projectTypes.index') }}" class="text-gray-900 dark:text-white">
                        Domain</a>
                </li>
            </ol>
        </div>
        ...
        <div class="px-2 py-2">
            ...
            {{-- create button --}}
            <div class="px-2 py-2 ">
                <a href="{{ route('projectTypes.create') }}"
                    ...
                    <div class="p-8 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                        <table class="w-full">
                            ...
                            <tbody>
                                ...
                                @php
                                    $starting_serial = ($projectTypes->currentPage() - 1) * $projectTypes->perPage() + 1;
                                @endphp
                                @foreach ($projectTypes as $key => $projectType)
                                    <tr class="border-t border-gray-200 dark:border-gray-700">
                                        <td class="px-4 py-3">{{ $starting_serial++ }}</td>
                                        <td class="px-4 py-3">{{ $projectType->name }}</td>
                                        <td class="px-4 py-3">
                                            <a href="{{ route('projectTypes.show', $projectType->id) }}"
                                            ...
                                            <a href="{{ route('projectTypes.edit', $projectType->id) }}"
                                            ...
                                            <form action="{{ route('projectTypes.destroy', $projectType->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    ...
                                                    onclick="return confirm('Are you sure you want to delete this projectType?')">
                                                    Delete
                                                </button>
                                            </form>
                                        </td>
                    ...
                    <div class="flex items-center justify-start mt-4">
                        <nav class="flex items-center">
                            ...
                            @else
                                ...
                                <a href="{{ $projectTypes->previousPageUrl() }}"
                            ...
                            <span class="px-2 py-1 text-sm font-medium leading-5 text-gray-600">
                                Page: {{ $projectTypes->currentPage() }} of {{ $projectTypes->lastPage() }} Records:
                                {{ $projectTypes->total() }}
                            </span>

                            {{-- Next Page Link --}}
                            @if ($projectTypes->hasMorePages())
                                <a href="{{ $projectTypes->nextPageUrl() }}"
                                ...
                                <a href="{{ $projectTypes->url($projectTypes->lastPage()) }}"
